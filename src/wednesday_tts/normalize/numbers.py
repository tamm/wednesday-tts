...
def normalize_fractions(text):
    ...
    """
    text = re.sub(r"\b(\d+)\s*-\s*(\d+)\s*/\s*(\d+)\b", r"\1 to \2 of \3", text)
...
def normalize_storage_units(text):
    ...
    text = re.sub(rf"\b({_NUM_PAT})(TB|GB|MB|KB|PB|tb|gb|mb|kb|pb)\b", storage_unit_to_speech, text)
    return text
...
def normalize_regular_decimals(text):
    ...
    """
    text = re.sub(r"(?<!v)(?<!\.)(\d+)\.(\d+)(?!\.\d)\b", r"\1 point \2", text)
    return text
